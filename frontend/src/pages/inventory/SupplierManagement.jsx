        ...
        <div className="w-full relative flex flex-col items-center mb-8">
          ...
          <div className="flex flex-col items-center">
            <h1 className="text-3xl font-extrabold text-white flex items-center gap-3 bg-white/25 backdrop-blur-md px-7 py-3.5 rounded-full border border-white/40 shadow-lg">
              <span className="text-white"><Truck size={28} /></span>
              Nhà Cung Cấp
            </h1>
            <p className="text-white/70 font-medium mt-3 text-center drop-shadow-sm">Quản lý danh sách nhà cung cấp thuốc.</p>
          </div>
          <button className="absolute right-0 top-1/2 -translate-y-1/2 bg-gradient-to-r from-sky-400 to-blue-400 hover:from-sky-300 hover:to-blue-300 text-slate-900 font-bold px-5 py-2.5 rounded-xl shadow-lg hover:shadow-sky-400/30 transition-all flex items-center gap-2" onClick={openCreate}>
        ...
        <div className="patient-glass-panel patient-glass-panel-clear rounded-[3rem] p-8 md:p-10 shadow-[0_12px_40px_rgba(0,0,0,0.22)] border-0 w-full">
          <div className="relative mb-6">
            ...
            <input
              ...
              className="w-full bg-slate-900/5 border border-slate-900/10 text-slate-900 placeholder-slate-500 text-sm rounded-xl py-3 pl-11 pr-4 focus:outline-none focus:border-sky-500/50 transition-colors backdrop-blur-sm font-bold"
            />
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-900/10 text-[#0f766e] text-sm">
                  <th className="p-4 font-bold pb-3">#</th>
                  <th className="p-4 font-bold pb-3">Tên nhà cung cấp</th>
                  <th className="p-4 font-bold pb-3">Số điện thoại</th>
                  <th className="p-4 font-bold pb-3">Email</th>
                  <th className="p-4 font-bold pb-3">Địa chỉ</th>
              ...
              <tbody className="text-slate-900">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-slate-900 font-bold">Không tìm thấy nhà cung cấp nào.</td>
                  </tr>
                ) : (
                  filtered.map((sup, idx) => (
                    <tr key={sup.supplierId} className="border-b border-slate-900/10 hover:bg-slate-900/5 transition-colors group">
                      <td className="p-4 font-bold">{idx + 1}</td>
                      <td className="p-4 font-bold text-slate-900">{sup.supplierName}</td>
                      <td className="p-4 font-bold">{sup.phone || "—"}</td>
                      <td className="p-4 font-bold">{sup.email || "—"}</td>
                      <td className="p-4 font-bold">{sup.address || "—"}</td>
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold border ${sup.status === "ACTIVE" ? "bg-emerald-500/20 text-emerald-700 border-emerald-500/30" : "bg-slate-900/10 text-slate-600 border-slate-900/20"}`}>
                          {sup.status === "ACTIVE" ? "Hoạt động" : "Ngừng HĐ"}
                        </span>
                      </td>
                      <td className="p-4 text-center">
                        <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button className="p-2 bg-slate-900/5 hover:bg-slate-900/10 rounded-lg text-slate-700 transition-colors" title="Chỉnh sửa" onClick={() => openEdit(sup)}>
                            <Edit size={16} />
                          </button>
                          <button
