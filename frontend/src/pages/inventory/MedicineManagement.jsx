    ...
    <>
      <div className="w-full flex flex-col items-center">
        <PageHeader
          ...
          subtitle="Quản lý danh sách các loại thuốc."
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-900/10 text-[#0f766e] text-sm">
                  ...
                  <th className="p-4 font-bold pb-3">Dạng bào chế</th>
                  <th className="p-4 font-bold pb-3">Đơn vị</th>
              ...
              <tbody className="text-slate-900">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-slate-900 font-bold">Không tìm thấy thuốc nào.</td>
                  </tr>
                ) : (
                  filtered.map((med) => (
                    <tr key={med.medicineId} className="border-b border-slate-900/10 hover:bg-slate-900/5 transition-colors group">
                      <td className="p-4 font-bold"><strong>{med.medicineCode}</strong></td>
                      <td className="p-4 font-bold text-slate-900">{med.medicineName}</td>
                      <td className="p-4 font-bold">{med.activeIngredient || "—"}</td>
                      <td className="p-4 font-bold">{med.dosageForm || "—"}</td>
                      <td className="p-4 font-bold">{med.unit || "—"}</td>
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold border ${med.status === "ACTIVE" ? "bg-emerald-500/20 text-emerald-700 border-emerald-500/30" : "bg-slate-900/10 text-slate-600 border-slate-900/20"}`}>
                          {med.status === "ACTIVE" ? "Đang bán" : "Ngừng bán"}
                        </span>
                      </td>
                      <td className="p-4 text-center">
                        <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button className="p-2 bg-slate-900/5 hover:bg-slate-900/10 rounded-lg text-slate-700 transition-colors" title="Chỉnh sửa" onClick={() => openEdit(med)}>
                            <Edit size={16} />
                          </button>
